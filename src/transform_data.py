...
def create_dataframe(path_name:str) -> pd.DataFrame:
  ...
  with open(path) as f:
     data = json.load(f) #Lê o texto json (dados brutos) e transforma em um objeto python (Lista ou dicionário)
...
def drop_column(df:pd.DataFrame, columns_names:list[str]) -> pd.DataFrame:
...
def normalize_datetime_column(df:pd.DataFrame, datetime_columns:list[str]) -> pd.DataFrame:
   logging.info(f"\n Convertendo colunas para datetime: {datetime_columns}")

   for name in datetime_columns:
      df[name] = pd.to_datetime(df[name], unit='s', utc = True).dt.tz_convert('America/Sao_Paulo')
      logging.info(f" Coluna convertida para datetime: {df[name]}\n") 
...
def data_transformation():
   ...
   df = drop_column(df,columns_name_to_drop)
   df = rename_columns(df, columns_names_to_rename)
   df = normalize_datetime_column(df, columns_to_normalize_datetime)
